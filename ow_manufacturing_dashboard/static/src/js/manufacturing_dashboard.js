...
export class OwManufacturingDashboard extends Component {
    static template = "ow_manufacturing_dashboard.Dashboard";
    ...
    // Keeping these in one place means the number shown on a card and the
    // list you land on after clicking it can never drift apart again.
    totalMODomain() {
        return [["state", "!=", "cancel"], ...this.getPeriodDomainFor("date_start")];
    }

    inProgressMODomain() {
        return [["state", "=", "progress"], ...this.getPeriodDomainFor("date_start")];
    }

    lateMODomain() {
        return [["state", "not in", ["done", "cancel"]], ["date_start", "<", this.nowStr()]];
    }

    doneTodayMODomain() {
        return [["state", "=", "done"], ["date_finished", ">=", this.startOfDayStr()]];
    }

    totalWODomain() {
        return [["state", "not in", ["done", "cancel"]], ...this.getPeriodDomainFor("date_start")];
    }
    ...
    async loadData() {
        ...
        const [
            ...
            this.orm.searchCount("mrp.production", this.totalMODomain()),
            this.orm.searchCount("mrp.production", this.inProgressMODomain()),
            this.orm.searchCount("mrp.production", this.lateMODomain()),
            this.orm.searchCount("mrp.production", this.doneTodayMODomain()),
            this.orm.searchCount("mrp.workorder", this.totalWODomain()),
            this.orm.searchCount("mrp.workorder", this.progressWODomain()),
            this.orm.searchCount("mrp.workorder", this.pendingWODomain()),
            this.orm.searchCount("mrp.workorder", this.lateWODomain()),
        ]);
	    ...
	    "mrp.production",
	    [["state", "not in", ["done", "cancel"]], ["date_start", "<", this.nowStr()]],
...
registry.category("actions").add("ow_manufacturing_dashboard_tag", OwManufacturingDashboard);
