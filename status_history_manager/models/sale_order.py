...
SALE_LINE_TRACKED = {
    ...
    'product_uom_qty': ('Ordered Quantity', 'quantity'),
...
class SaleOrder(models.Model):
    ...
    def write(self, vals):
        ...
        old_values = {}
        fields_changing = [f for f in SALE_ORDER_TRACKED if f in vals]
        if fields_changing:
            for record in self:
        ...
        if old_values:
            ...
            for record in self:
                if record.id not in old_values:
                    continue
                for fn, (label, ctype) in SALE_ORDER_TRACKED.items():
                    if fn not in vals:
                        continue
...
class SaleOrderLine(models.Model):
    ...
    def write(self, vals):
        ...
        old_values = {}
        fields_changing = [f for f in SALE_LINE_TRACKED if f in vals]
        if fields_changing:
            for record in self:
                old_values[record.id] = {}
                for fn in fields_changing:
        ...
        if old_values:
            ...
            for record in self:
                ...
                if not order:
                    continue
                for fn, (label, ctype) in SALE_LINE_TRACKED.items():
                    if fn not in vals:
                        continue
                    old_d = old_values[record.id].get(fn, '')
                    new_d = self._history_get_display_value(fn, record[fn])
